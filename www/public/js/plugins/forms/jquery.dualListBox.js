...
(function ($) {
    ...
    $.configureBoxes = function (options) {
        var index = settings.push({
            ...
            to1: 'to1', allTo1: 'allTo1',
            to2: 'to2', allTo2: 'allTo2',
            transferMode: 'move',
            sortBy: 'text',
            useFilters: true,
            useCounters: true,
        ...
        if (settings[index].useFilters) {
            $('#' + group1[index].filter).on("keyup", (function () {
                Filter(group1[index]);
            }));
            $('#' + group2[index].filter).on("keyup", (function () {
                Filter(group2[index]);
            }));
            $('#' + group1[index].clear).on("click",(function () {
                ClearFilter(group1[index]);
            }));
            $('#' + group2[index].clear).on("click",(function () {
                ClearFilter(group2[index]);
            }));
        }
        if (IsMoveMode(settings[index])) {
            $('#' + group2[index].view).on("dblclick", (function () {
                MoveSelected(group2[index], group1[index]);
            }));
            $('#' + settings[index].to1).on("click",(function () {
                MoveSelected(group2[index], group1[index]);
            }));
            $('#' + settings[index].allTo1).on("click",(function () {
        ...
        $('#' + group1[index].view).on("dblclick", (function () {
            MoveSelected(group1[index], group2[index]);
        }));
        $('#' + settings[index].to2).on("click",(function () {
            MoveSelected(group1[index], group2[index]);
        }));
        $('#' + settings[index].allTo2).on("click",(function () {
        ...
        if (settings[index].selectOnSubmit) {
            $('#' + settings[index].box2View).closest('form').on("submit", (function () {
    ...
    function RemoveAll(removeGroup, otherGroup) {
        ...
        catch (ex) { } Filter(otherGroup);
        if (settings[removeGroup.index].useCounters) { UpdateLabel(removeGroup); }
    }
    function ClearFilter(group) {
        $('#' + group.filter).val(''); $('#' + group.storage + ' option').appendTo('#' + group.view);
        try {
            $('#' + group.view + ' option').removeAttr('selected');
        }
        catch (ex) { }
