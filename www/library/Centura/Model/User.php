...
class Centura_Model_User extends Centura_Model_DbTable_User
{
	...
	public function fetchuser($username,$company=DEFAULT_COMPNAY_ID)
	{
	    ...
	    $db = $this->getAdapter();
	    $select = $db->select()->from("P21_Users",array('id','name','company'=>'default_company','default_branch','role_uid','email'=>'email_address'))->where('P21_Users.delete_flag = ?','N')
	    ->join('P21_Roles','P21_Roles.role_uid = P21_Users.role_uid',array('role'))->join('P21_Location','P21_Location.company_id = P21_Users.default_company','location_id')->where('P21_Users.id = ?',$username);
	    if($company != null)
	    {
	    ...
	    if(strpos($user['role'],'Sales' ) !== false 
		...
		if(strpos( $user['role'] , 'Sales Management') !== false || strpos($user['role'],'Sales Admin') !== false 
	    ...
	    $db = $this->getAdapter();
	    $select = $db->select()->from("quote_approval",'uid')->where('Userid = ?',$username);
	...
	public function fetchallusers($company = DEFAULT_COMPNAY_ID)
	{
		...
		$select = $db->select()->from("P21_Users",array('id','name','default_company','role_uid','email'=>'email_address'))->where('P21_Users.delete_flag = ?','N')
				...
				->join('P21_Location','P21_Location.company_id = P21_Users.default_company','location_id');
		if($company != null)
		{
			$select->where('P21_Location.location_id = ?',$company);
		}
	...
	public function get_role($username,$company=DEFAULT_COMPNAY_ID)
	{
	    ...
	    $select = $db->select()->from("P21_Users",array('id','name','default_company','role_uid','email'=>'email_address'))->where('P21_Users.delete_flag = ?','N')
			    ->join('P21_Roles','P21_Roles.role_uid = P21_Users.role_uid',array('role'))
			    ->join('P21_Location','P21_Location.company_id = P21_Users.default_company','location_id')->where('P21_Users.id = ?',$username);
	    if($company != null)
	    {
	    	$select->where('P21_Location.location_id = ?',$company);
	    }
	...
	public function fetchallsalesbyloc($company = DEFAULT_COMPNAY)
	{
		...
		if($company != null)
		{
			$select->where('P21_Location.company_id = ?',$company);
		}
		return $db->fetchAll($select);
	}
	...
	public function fetchsalebyid($id)
	{
		...
		$db = $this->getAdapter();
		$select = $db->select()->distinct()->from("P21_Users",array('id','name','default_company','default_location_id'))->where('id = ?',$id);
	...
	public function fetchlocationid($company_id)
	{
		...
		$db = $this->getAdapter();
		$select = $db->select()->from("P21_Location",array('location_id'))->where('company_id = ?',$company_id);
		return $db->fetchRow($select);
