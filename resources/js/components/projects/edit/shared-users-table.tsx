...
import ShareProjectButton from './shared-users-table-share-button';

export default function ProjectSharedUsersTable() {
    const projectId = window.location.pathname.split('/')[2];
    const ENDPOINT = '/project-shares';
    const qKey = ['project-shares', projectId];

    const queryClient = useQueryClient();
    const { data: users = [], isLoading, isRefetching } = useTanStackQuery<ProjectShare>(ENDPOINT + `?project=${projectId}`, qKey);
    ...
    const columns: ColumnDef<ProjectShare>[] = [
        {
