...
class Opportunity extends Model
{
    ...
    protected function prepareDataForCreate(array $data): array
    {
        $info = [
            ...
            'sample_submitted'      => ! empty(trim($data['sample_submitted'])) ? trim($data['sample_submitted']) : 0,
            ...
            'start_date'            => ! empty($data['require_date']) ? $data['require_date'] : null,
            'completion_date'       => ! empty($data['due_date']) ? $data['due_date'] : null,
        ];

        if (! empty($data['architect_name'])) {
