...
bobj.crv.GroupTree.delayedBatchAdd = function(children) {
    var batchSize = 25;
    var timeOutLength = 50;
    var treeWidget = this;

    var updateFunction = function(startIndex, endIndex) {
        for (var i = startIndex; i < endIndex; i++)
        {
            var node = bobj.crv.createWidget(children[i]);
            treeWidget.addChild(node);
            append(treeWidget.layer, node.getHTML());
        }

        var newEndIndex = (endIndex + batchSize < children.length ) ? endIndex + batchSize : children.length;
        if (newEndIndex - endIndex > 0)
            setTimeout(MochiKit.Base.bind(arguments.callee, window, endIndex, newEndIndex), timeOutLength);
    }

    
    var endIndex = ( batchSize < children.length ) ? batchSize : children.length;
    setTimeout(MochiKit.Base.bind(updateFunction, window, 0, endIndex), timeOutLength);
}
