...
bobj.crv.ReportAlbum.resizeOuter = function(w, h) {
    if (this.displayDrilldownTab && bobj.isNumber (h))
        h -= 33;
    
    if (!this._hideFrame && bobj.isNumber (w))
        w -= 10;
...
/**
 * @return Returns true if drill down tabs are displayed
 */
bobj.crv.ReportAlbum.isDisplayDrilldownTab = function() {
    return this.displayDrilldownTab;
};

bobj.crv.ReportAlbum.setHideFrame = function(hide) {
	this._hideFrame = hide;
	if (this._curView && this._hideFrame)
    	    this._curView.hideFrame();
}
/**
